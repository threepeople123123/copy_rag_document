...
class MinioClient:
    ...
    def __init__(self):
        # minio>=7.2 会在内部按 secure 自动拼接 http:// 或 https:// 前缀，
        # endpoint 不能再带 scheme，否则会拼成 http://http://host:port 而报
        # "path in endpoint is not allowed"。这里做一次归一化兜底：去掉 scheme 和末尾斜杠。
